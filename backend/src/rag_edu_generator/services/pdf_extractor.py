...
from rag_edu_generator.utils.errors import PDFExtractionError
...
class PDFExtractor:
    ...
    def extract_from_bytes(
        ...
        Returns:
            ExtractedDocument with text, page count, and metadata
        ...
        try:
            ...
            if not full_text_stripped or len(full_text_stripped) < 10:
                ...
                error_msg = (
                    ...
                    f"This PDF appears to be image-based (scanned) or contains no text layer. "
                    f"Pages processed: {page_count}, Pages with text: {pages_with_text}, "
                    f"Total characters extracted: {total_chars}. "
                    f"Please ensure your PDF has selectable text (not just images)."
                )
                if filename:
            ...
            logger.info(
                f"Successfully extracted {page_count} pages from PDF "
                f"({pages_with_text} pages with text, {total_chars} characters)"
            ...
            return ExtractedDocument(
                ...
                chunks=[],  # Chunks will be created by chunking service
                metadata=metadata,
            )
    ...
    def extract_from_file(self, file_path: str) -> ExtractedDocument:
        ...
        try:
            ...
            raise PDFExtractionError(
                f"Error reading PDF file {file_path}: {str(e)}"
            ) from e
